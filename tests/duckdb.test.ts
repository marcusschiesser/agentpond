...
test("DuckDB sync is idempotent and projects traces, sessions, scores, and raw events", async () => {
  ...
  const traces = await db.query<{ id: string; total_cost: number | null }>("select id, total_cost from traces");
  ...
  assert.deepEqual(observations, [
    {
      ...
      cost_details_json: JSON.stringify({ input: 0.01, output: 0.02, total: 0.03 }),
      total_cost: 0.03,
    },
    {
      id: "observation-usage-only",
  ...
  assert.deepEqual(JSON.parse(rawRows[0].body_json).costDetails, { input: 0.01, output: 0.02, total: 0.03 });
    ...
    [
      {
        ...
        timestamp: "2026-06-14T00:00:00.000Z",
        type: eventTypes.TRACE_CREATE,
        body: { id: "trace-1", sessionId: "session-a", startTime: "2026-06-14T00:00:00.000Z" },
      },
      {
        id: "trace-event-2",
        timestamp: "2026-06-14T00:01:00.000Z",
        type: eventTypes.TRACE_CREATE,
        body: { id: "trace-2", sessionId: "session-b", startTime: "2026-06-14T00:01:00.000Z" },
      },
    ],
    "batch-1",
  );
