...
description: Add OpenInference tracing to trusted Firebase, Supabase, or Vercel applications and export spans directly to AgentPond storage. Use when instrumenting an untraced AI server, adding a missing OpenInference integration, or adapting existing OpenTelemetry setup for provider-owned object storage.
---
...
1. Detect Firebase, Supabase, or Vercel project markers. If multiple exist, ask which platform owns the deployed service; do not persist that choice.
2. Confirm which trusted Node.js service or Supabase Edge Function is in scope. In a monorepo, do not assume every server package should be instrumented.
...
2. Install the provider package (`@agentpond/firebase`, `@agentpond/supabase`, or `@agentpond/vercel`), required OpenTelemetry packages, and the matching `@arizeai/openinference-*` package.
...
Treat the work as complete only when the project builds or typechecks, the trusted runtime loads without duplicate-provider errors, one real AI request exports OpenInference spans, and the trace appears after the provider-specific sync command in its reference.
...
This workflow is adapted from Arize AI's MIT-licensed [arize-instrumentation skill](https://github.com/Arize-ai/arize-skills/tree/main/skills/arize-instrumentation). It replaces Arize-specific export and verification with AgentPond direct export to Firebase Storage, Supabase Storage, or Vercel Blob while retaining the analyze-then-implement workflow.
