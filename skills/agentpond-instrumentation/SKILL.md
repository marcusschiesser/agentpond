...
3. Review the selected storage path's privacy and credential requirements from
   ...
   policy; if none exists, use the exporter's default metadata-only tracing.
...
Read back the raw stored object to verify the exporter policy. Under the default
...
search for the representative prompt, response, tool payload, and provider
error text to prove they are not recoverable. If content capture is explicitly
enabled, verify only the approved fields and still reject credentials or
